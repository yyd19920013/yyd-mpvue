...
// 结束咨询订单
const endConsult = ({orderDetailId,itemCode},success) => {
    return IHRequest({
        url: '/cas_ih_foshan.wx_consultService/endConsult',
        method: 'post',
        data:[{
            orderDetailId,
            itemCode,
...
// 结束复诊订单
const endRevisit = (revisitId,success) => {
    return IHRequest({
...
const confirmReceiptDrug = (orderNo,success) => {
    return IHRequest({
        url: '/cas_ih_foshan.wx_recipeService/confirmReceipt',
        method: 'post',
        data:[{orderNo}],
        success,
    });

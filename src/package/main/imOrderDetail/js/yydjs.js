...
var lStore = {
    set: function () {
        let result = wx.setStorageSync.apply(this, arguments);

        return result;
    },
    get: function () {
        let result = wx.getStorageSync.apply(this, arguments);

        return result;
    },
    getAll: function () {
        let result = wx.getStorageInfoSync.apply(this, arguments);

        return result;
    },
    remove: function () {
        let result = wx.removeStorageSync.apply(this, arguments);

        return result;
    },
    clear: function () {
        let result = wx.clearStorageSync.apply(this, arguments);
...
//微信小程序-给当前页面定义一个路由对象，用于获取quey和跳转，会给当前页面this加上$router和$query
function addRouter(parent) {
    if (!parent) return console.error('使用addRouter方法时必须传入parent参数，值为组件的this');
    var $mp = parent.$mp;
    var $router = {
        push: function (pageType, option) {
            var pushJson = {
                switch: 'switchTab',
                navigate: 'navigateTo',
                redirect: 'redirectTo',
            };
            var pushApi = pushJson[pageType];
            var type = Type(option);
            var option = type == 'string' ? { path: option } : option;
            ...
            wx[pushApi]({
                url: `${path}${queryStr.length?'?':''}${queryStr}`,
                success: success,
                fail: fail,
                complete: complete,
            });
        ...
        switch: function (option) {
            this.push('switch', option);

            return this;
        },
        navigate: function (option) {
            this.push('navigate', option);

            return this;
        },
        redirect: function (option) {
            this.push('redirect', option);

            return this;
        },
        back: function (option) {
            var type = Type(option);
            var option = type == 'number' || type == 'string' ? { delta: option } : option;
...
function wxxcxShare(option) {
    ...
    return {
        ...
        imageUrl: option.imageUrl || require('images/loading.gif'),
